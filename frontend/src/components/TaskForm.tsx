...
const TaskForm = ({ task, onClose, onSubmit }: TaskFormProps) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (formData.scheduledDate) {
      submitData.scheduledDate = new Date(formData.scheduledDate).toISOString();
    }
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end space-x-3 pt-4">
            <button
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-100 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
